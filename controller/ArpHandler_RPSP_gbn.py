...
class ArpHandler(app_manager.RyuApp):
    ...
    def create_Cap_Mat(self):
        G = nx.read_gml('/home/hao/thesis_code/Topology and data_generation/NetworkX_graph_file/gbnnet/gbn.txt', destringizer=int)
    ...
    def set_shortest_path(self,
        ...
        if len(path) == 1:
            ...
            priority = random.randint(10,60000)
            self.add_flow(dst_dp, priority, to_dst_match, pre_actions+actions)
    ...
    def get_linkbw_matrix(self):
        ...
        if total_util > 0.01:
            ###################### Write Util ###########################
            with open('gbn_result_RPSP_10ms.txt','a') as f:
            ...
            ###################### Write Throughput ###########################
            with open('gbn_result_RPSP_10ms_bw.txt','a') as f:
