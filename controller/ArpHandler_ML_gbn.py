...
class ArpHandler(app_manager.RyuApp):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.tmp_flow_rate_matrix = []
        self.flow_rate_matrix = []
    ...
    def _predict_thread(self):
        while(1):
            now_time = time.time()
            if(now_time - self.global_time) >= 15000000:
    ...
    def create_Cap_Mat(self):
        ...
        self.switch_port_to_switch = np.full(
            (len(self.cap_mat), len(self.cap_mat)), 0)
        self.tmp_flow_rate_matrix = np.full((len(self.cap_mat), len(self.cap_mat)), 0)
        self.flow_rate_matrix = np.full((len(self.cap_mat), len(self.cap_mat)), 0)
        self.input_rate_matrix = np.full((len(self.cap_mat), len(self.cap_mat)), 0)
    ...
    def predictor(self, load=0):
        ...
        with graphPredict.as_default():
            if self.built == 0:
                hparams = rout.hparams.parse(
                    "l2=0.01,dropout_rate=0.0,link_state_dim=64,path_state_dim=64,readout_units=256,learning_rate=0.0001,T=4")
            ...
            with tf.name_scope('predict'):
                t1 = time.time()
                self.predictions = tf.map_fn(lambda x: self.model(
                    x, training=False), features, dtype=tf.float32)
        ...
        with tf.compat.v1.Session(graph=graphPredict) as sess:
            ...
            for i in range(182):
                path_dict[str(self.all_pairs_shortest_path[i])
                          ] = final_prediction[i]
            sorted_path_dict = {k: v for k, v in sorted(
                path_dict.items(), key=lambda item: item[1])}
    ...
    def make_tfrecord(self):
        ...
        self.link_capacities = self.link_capacities
    ...
    def make_indices(self, src, dst):
        ...
        for spath in self.all_pairs_shortest_path:
            ...
            self.traffic.append(self.flow_rate_matrix[spath[i]], spath[i+1])
    ...
    def get_linkbw_matrix(self):
        ...
        # print('LINKBW_MATRIX = \n', self.linkbw_matrix_now)
        self.input_rate_matrix = (self.tmp_flow_rate_matrix - self.flow_rate_matrix) /2
        self.flow_rate_matrix = np.copy(self.tmp_flow_rate_matrix)    
        ...
        if total_util > 0.01:
            ###################### Write Util ###########################
            with open('gbn_result_ML_50ms.txt', 'a') as f:
            ...
            ###################### Write Throughput ###########################
            with open('gbn_result_ML_50ms_bw.txt', 'a') as f:
    ...
    def _flow_stats_reply_handler(self, ev):
        ...
        # pattern_dst = re.compile(r'\'ipv4_dst\': .\d+\.\d+\.\d+\.\d+\.\'')
        pattern_dst = re.compile(r'\'ipv4_dst\': .(\d+\.\d+\.\d+\.\d+)')
        for flow in body:
            if flow.priority == 10:

                match = str(flow.match)
                ip_list_type = pattern_dst.findall(match)
                # print(match)
                ip_str = ''
                for x in ip_list_type:
                    ip_str += x
                # print(ip_str)

                # self.logger.info('%016s %17s %8d %8d',
                #                  ev.msg.datapath.id,
                #                  ip_str,
                #                  flow.packet_count, flow.byte_count)

                self.tmp_flow_rate_matrix[int(ev.msg.datapath.id), int(ip_str.split('.')[-1])] = flow.byte_count
            print('tmp flow rate mat = \n', self.tmp_flow_rate_matrix)
        # print('input rate matrix : \n', (self.tmp_flow_rate_matrix))
